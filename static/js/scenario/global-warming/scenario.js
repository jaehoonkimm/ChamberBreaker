...
import CountUp from '../../game/countUp.min.js';
...
class GlobalWarming {
  ...
  init(num) {
    ...
    sUtils.init('global-warming', tweets, tweetData, retweets, nwTweet);
  ...
  nextScenario = () => {
    ...
    if (num == 0) {
      ...
      badge.updateGlobalWarmingScore(this.scenarioScore, this.scenarioNum);
  ...
  countUpView = () => {
    ...
    const countUp = new CountUp('viewCountUp', 1716, options);
    if (!countUp.error) {
      countUp.start();
    } else {
...
const globalWarming = new GlobalWarming();
